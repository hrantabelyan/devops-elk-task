...
└── ansible/
    ...
    ├── site.yml           elk play (elk role), then web play (apache role)
    ├── group_vars/all.yml Elastic Stack passwords, generated into .secrets/ on first run
    ├── roles/
...
```bash
ansible-playbook site.yml --limit web -e apache_acme_terms_agreed=true
```
